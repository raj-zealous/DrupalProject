...
{ 
	...
	*    - arg - 1 - array - receipents, subject, body, self copy
    *
	*/
	function sendMessage( $arg = array() )
	{   
		try{
				$response  = $this->api->message( $arg[0], $arg[1], $arg[2], $arg[3]);

			}
		catch( LinkedInException $e ){
			throw new Exception( "Send Message Failed! {$this->providerId} returned an error: $e" );
		}
	...
	*/
    ...
    function sendInvite( $arg = array() )
    {  
         try{
            $response  = $this->api->invite( $arg[0], $arg[1], $arg[2], $arg[3], 'friend' );
         }
         catch( LinkedInException $e ){
            throw new Exception( "Send Invitation Failed! {$this->providerId} returned an error: $e" );
         }
         return $response;
    }
...
/**
	* send linkedin invite
	*    - arg - 1 - array - method(email, id), receipent, Subject,
    *    body, type(friend)
    *
	*/
	function sendInvite( $arg = array() )
	{  
		try{
			...
			$response  = $this->api->invite( $arg[0], $arg[1], $arg[2],
            $arg[3], 'friend' );
		}
		catch( LinkedInException $e ){
			throw new Exception( "Send Invitation Failed! {$this->providerId} returned an error: $e" );
